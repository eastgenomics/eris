...
class TestSanityCheckColsExist(TestCase):
    ...
    def test_mane_missing_gene(self):
        ...
        with self.assertRaisesRegex(AssertionError, "Missing column Gene from MANE file - please check the file"): 
    ...
    def test_mane_missing_mane_type(self):
        ...
        with self.assertRaisesRegex(AssertionError, "Missing column MANE TYPE from MANE file - please check the file"): 
                    _sanity_check_cols_exist(test_mane, self.mane_cols, self.mane_name)
    
    def test_mane_missing_refseq(self):
        test_mane = pd.DataFrame({"Gene": [], "MANE TYPE": []})
        with self.assertRaisesRegex(AssertionError, "Missing column RefSeq StableID GRCh38 / GRCh37 from MANE file - please check the file"): 
            _sanity_check_cols_exist(test_mane, self.mane_cols, self.mane_name)
