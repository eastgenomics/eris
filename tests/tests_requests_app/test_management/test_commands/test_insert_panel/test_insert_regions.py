...
class TestInsertRegions_NewRegion(TestCase):
    ...
    def test_new_panel_linked_to_acceptable_region(self):
        ...
        errors += value_check(first_panel_regions.panel, "first panel-region panel", 
                              self.first_panel)
        errors += value_check(first_panel_regions.region, "first panel-region region", 
                              regions[0])
        errors += value_check(second_panel_regions.panel, "second panel-region panel", 
                              self.first_panel)
        errors += value_check(second_panel_regions.region, "second panel-region region", 
                              regions[1])
...
class TestInsertRegions_PreexistingLink(TestCase):
    ...
    def test_pre_existing_panel_region_link(self):
        ...
        panel_regions = PanelRegion.objects.all()
        errors += len_check(panel_regions, "panel-regions", 2)
        errors += value_check(panel_regions[0], "first panel-region", 
