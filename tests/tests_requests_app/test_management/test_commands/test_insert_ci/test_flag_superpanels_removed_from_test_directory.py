...
from requests_app.management.commands._insert_ci import _flag_superpanels_removed_from_test_directory
...
class TestSuperpanelsFlaggedWhenNoLongerInTd(TestCase):
    ...
    def test_flag_superpanels_removed_from_test_directory(self):
        ...
        Current Superpanels which ARE in the td-files should be skipped, remaining current with no history data
        """
        _flag_superpanels_removed_from_test_directory(self.ci, self.current_td_panels, "test user")
        ...
        superpanel_ext_3 = ClinicalIndicationSuperPanel.objects.filter(superpanel__external_id="3")
        with self.subTest():
        ...
        superpanel_ext_4 = ClinicalIndicationSuperPanel.objects.filter(superpanel__external_id="4")
        with self.subTest():
