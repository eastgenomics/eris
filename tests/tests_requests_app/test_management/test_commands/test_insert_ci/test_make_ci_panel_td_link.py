...
"""
CASES:
- CI panel already existed
    It is linked to a TdRelease
    It is logged as a new link in the CIP history
- CI panel created
    It is linked to a TdRelease
    It is logged as metadata-changed in the CIP history
"""
...
class TestMakeCiPanelTdLink_NewCip(TestCase):
    ...
    def test_panel_ci_and_td_link_made(self):
        ...
        """
        cip, cip_created = _make_ci_panel_td_link(self.ci, self.panel, self.td_release, self.user)
        ...
        with self.subTest():
            ...
            self.assertEqual(cip_hist[0].note, History.clinical_indication_panel_created())

        # check cip-td  history logs
        with self.subTest():
            cip_td_hist = CiPanelTdReleaseHistory.objects.all()
            assert len(cip_td_hist) == 1
            self.assertEqual(cip_td_hist[0].note, History.td_panel_ci_autolink(cip_td[0].td_release.release))
...
class TestMakeCiPanelTdLink_ExistingCip(TestCase):
    ...
    def test_td_link_made(self):
        ...
        change
        """
        new_cip, cip_created = _make_ci_panel_td_link(self.ci, self.panel, self.td_release, self.user)
        ...
        with self.subTest():
            ...
            assert len(hist) == 1
            self.assertEqual(hist[0].note, History.td_panel_ci_autolink(
