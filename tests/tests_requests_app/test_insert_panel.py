...
class TestFlagActiveLinksForPanel(TestCase):
    ...
    def test_flag_ci_panel_instances_controller(self):
        ...
        Check that the CI in the database is changed to 'needs review' and has history logged
        """
        ...
        ci_panel_instances = flag_active_links_for_panel(new_panel.external_id, 
                                                         "test_user")

        # Expect a QuerySet of 1 ClinicalIndicationPanel result, because this function only flags 
        # existing CI-Panel links, 
        # and the new panel hasn't had one made yet
        ...
        ci_first = ci_panel_instances.all()[:1].get()
        assert ci_first.config_source == "unit_test"
        assert ci_first.needs_review == True

        # TODO: Check for a history record for ClinicalIndicationPanel
