...
class TestGenerateG2tResults(TestCase):
    def setUp(self) -> None:
        ...
        self.grch37 = ReferenceGenome(
            name="GRCh37"
        )
        self.grch38 = ReferenceGenome(
            name="GRCh38"
        )
        ...
        self.mane_select = TranscriptSource(
            source="MANE Select"
        )
        self.mane_plus_clinical = TranscriptSource(
            source="MANE Plus Clinical"
        )
        self.hgmd = TranscriptSource(
            source="HGMD"
        )
