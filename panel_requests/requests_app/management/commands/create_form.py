...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("hgnc_dump", nargs=1, help="Name of HGNC dump text file")
    ...
    def get_panel_records(self, ci_code):
        ...
        panel_records = Panel.objects.filter(
            ...
            clinicalindicationpanel__current=True,
        ).values()
    ...
    def get_gene_records(self, panel_dict):
        ...
        panel_genes = PanelGene.objects.filter(panel_id=panel_dict["id"]).values()
        ...
        if len(panel_genes) > 0:
            for gene in panel_genes:
                ...
                hgnc = Hgnc.objects.filter(gene__panelgene__id=gene["id"]).values()[0][
                    "id"
                ]
    ...
    def get_region_records(self, panel_dict):
        ...
        panel_regions = PanelRegion.objects.filter(panel_id=panel_dict["id"]).values()
        ...
        if len(panel_regions) > 0:
            for region in panel_regions:
                ...
                name = Region.objects.filter(panelregion__id=region["id"]).values()[0][
                    "name"
                ]
                ...
                start = Region.objects.filter(panelregion__id=region["id"]).values()[0][
                    "start"
                ]

                end = Region.objects.filter(panelregion__id=region["id"]).values()[0][
                    "end"
                ]
        ...
        base_df = pd.DataFrame(
            {
                ...
                "data": [req_date, requester, ci_code, str(dt.today())],
            }
        )
    ...
    def write_data(self, fp, generic_df, panel_df, gene_df, region_df):
        ...
        generic_df.to_excel(
            writer, sheet_name="Sheet1", startrow=row, startcol=col, header=False
        )
        ...
        panel_df.to_excel(
            writer, sheet_name="Sheet1", startrow=row, startcol=col, index=False
        )
    ...
    def format_excel(self, excel_file, header_rows):
        ...
        highlight.fill = PatternFill(
            fill_type="solid", start_color="00FFCC00", end_color="00FFCC00"  # yellow
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        if req_date and requester and ci_code and hgnc_dump:
            ...
            generic_df = self.create_generic_df(req_date, requester, ci_code)
            ...
            headers = self.write_data(file, generic_df, panel_df, gene_df, region_df)
            ...
            print(
                "The following arguments are required, and must be "
