        ...
        gene_df = pd.DataFrame(
            {
                ...
                "confs": [cell[0].value for cell in ws[f"F{g_start}":f"F{g_end}"]],
                "pens": [cell[0].value for cell in ws[f"G{g_start}":f"G{g_end}"]],
                "mops": [cell[0].value for cell in ws[f"H{g_start}":f"H{g_end}"]],
                "mois": [cell[0].value for cell in ws[f"I{g_start}":f"I{g_end}"]],
            }
        )
